...
void main() {
  testWidgets('Sequence animation smoke test', (WidgetTester tester) async {
    ...
    final seqKey = SequenceAnimationTag.random<Color?>();
...
//https://docs.flutter.io/flutter/package-test_test/throwsA.html
  ...
  testWidgets('Sequence animation with 4 colors', (WidgetTester tester) async {
    ...
    SequenceAnimation sequenceAnimation = SequenceAnimationBuilder()
        ...
        .addAnimatable(
            tag: seqKey,
            animatable: ColorTween(begin: Colors.green, end: Colors.deepPurple),
            from: const Duration(seconds: 2),
            to: const Duration(seconds: 3))
        .addAnimatable(
            tag: seqKey,
            animatable: ColorTween(begin: Colors.deepPurple, end: Colors.brown),
  ...
  testWidgets('Sequence animation with 4 colors add after API',
  ...
  testWidgets('Sequence with no animations', (WidgetTester tester) async {
    ...
    SequenceAnimation sequenceAnimation =
        SequenceAnimationBuilder().animate(controller);

    expect(controller.duration, equals(const Duration(seconds: 0)));

    try {
      sequenceAnimation.get(SequenceAnimationTag.random<bool>());
    } catch (e) {
  ...
  testWidgets('Sequence animation with 4 colors', (WidgetTester tester) async {
    ...
    final colorKey = SequenceAnimationTag.random<Color?>();
    final widthKey = SequenceAnimationTag.random<double>();
    ...
    SequenceAnimation sequenceAnimation = SequenceAnimationBuilder()
        ...
        .addAnimatable(
            tag: widthKey,
            animatable: Tween(begin: 50.0, end: 500.0),
            from: const Duration(seconds: 1),
    ...
    BoxConstraints constraints =
        tester.widget<Container>(find.byKey(key)).constraints!;
  ...
  testWidgets('invalide sequence', (WidgetTester tester) async {
    ...
    final tag = SequenceAnimationTag.random<Color?>();
