...
import { RevenueCatEvent } from "server/engine/webhook/revenue-cat-event";
...
async function updateUserEntitlement(eventJSON: Record<string, unknown>) {
  const subscriptionService = diContainer.get<SubscriptionService>(
    TYPES.SubscriptionService
  );
  try {
    const revenueCatEvent = RevenueCatEvent.from(eventJSON);
    await subscriptionService.processRevenueCatEvent(revenueCatEvent);
  } catch (e) {
    const { name, message, stack } = e as Error;
    logger.error({
      op: "USER_ENTITLEMENT_UPDATE_FAILED",
      message: {
        revenueCatEvent: eventJSON,
...
const processWebhook = async (
  ...
  await updateUserEntitlement(reqBody);
