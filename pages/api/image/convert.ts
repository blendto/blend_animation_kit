...
import { convertUnspportedFormatToWebp } from "../../../server/helpers/imageUtils";
...
const convertIncomingImage = async (
  ...
  if (fileNameArr.length <= 1) {
    ...
    validImageBuffer = await (await sharpInstance(imageBuffer))
      .toFormat("webp")
      .toBuffer();
  } else {
    const fileExtension = fileNameArr.pop();
    if (VALID_UPLOAD_IMAGE_EXTENSIONS.includes(fileExtension)) {
      validImageBuffer = await (
        await sharpInstance(imageBuffer, {}, fileExtension)
      )
        .toFormat("webp")
        .toBuffer();
    } else {
      validImageBuffer = await convertUnspportedFormatToWebp(
        imageBuffer,
        fileName
      );
    }
  ...
  return res.send(validImageBuffer);
};
